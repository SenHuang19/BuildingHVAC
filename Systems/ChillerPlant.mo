...
model ChillerPlant "a typical chiller plant"
  ...
  parameter Modelica.SIunits.Power PTow_nominal[:] = {10E3 for i in linspace(1,n,n)}
  ...
  parameter Modelica.SIunits.MassFlowRate mCHW_flow_nominal[:] = {-datChi[1].QEva_flow_nominal/4200/5.56 for i in linspace(1, n, n)}
    "Nominal mass flow rate at chilled water side";
  parameter Modelica.SIunits.MassFlowRate mCW_flow_nominal[:] = {mCHW_flow_nominal[1]*(datChi[1].COP_nominal+1)/datChi[1].COP_nominal for i in linspace(1, n, n)}
  ...
  parameter Real v_flow_rate[m,:] = {{0.4*sum(mCHW_flow_nominal)/m/996,0.6*sum(
      ...
      m,
      m)};
  parameter Real pressure[m,:] = {{2.2*dP_nominal,1.2*dP_nominal,1.1*dP_nominal,dP_nominal,0.75*dP_nominal} for i in linspace(1, m, m)};
  parameter Real Motor_eta_Sec[m,:] =  {{0.6,0.76,0.87,0.86,0.74} for i in linspace(1, m, m)}
    "Motor efficiency";
  parameter Real Hydra_eta_Sec[m,:] = {{1,1,1,1,1} for i in linspace(1, m, m)}
    "Hydraulic efficiency";
  ...
  Subsystems.Pump.PumpSystem            pumSecCHW(
    ...
    m_flow_nominal={sum(mCHW_flow_nominal)/m for i in linspace(
        ...
        m,
        m)},
    dp(displayUnit="Pa") = {dP_nominal*0.5 for i in linspace(
      1,
      m,
      m)})
  ...
  IBPSA.Fluid.Sensors.TemperatureTwoPort senTCHWByp(
    redeclare package Medium = MediumCHW,
    ...
    m_flow_nominal=sum(mCHW_flow_nominal))
        ...
        rotation=90,
        origin={0,-10})));
  IBPSA.Fluid.Storage.ExpansionVessel expVesCHW(
      redeclare package Medium = MediumCHW)
    annotation (Placement(transformation(extent={{-44,-78},{-36,-70}})));
  IBPSA.Fluid.Sensors.MassFlowRate senMasFloByp(redeclare package Medium =
  ...
  replaceable Devices.WaterSide.Control.PlantStageN       chillerStage(
    ...
    n=n,
    thehol=thrhol,
    Cap=Cap) annotation (Placement(transformation(extent={{-82,58},{-62,78}})));
  Subsystems.Pump.Control.SecPumCon         secPumCon(          dPSetPoi=
        dP_nominal*0.5,
    tWai=1800,
    n=m)
    annotation (Placement(transformation(extent={{60,42},{80,62}})));
  replaceable IBPSA.Fluid.Sensors.TemperatureTwoPort senTCHWBuiEnt(
    redeclare package Medium = MediumCHW,
    allowFlowReversal=true,
    m_flow_nominal=sum(mCHW_flow_nominal))
                            annotation (Placement(transformation(
  ...
  IBPSA.Fluid.Sensors.TemperatureTwoPort senTCHWBuiLea(
    redeclare package Medium = MediumCHW,
    allowFlowReversal=true,
    m_flow_nominal=sum(mCHW_flow_nominal)) annotation (Placement(transformation(
        ...
        rotation=0,
        origin={70,18})));
  replaceable IBPSA.Fluid.Sensors.MassFlowRate senMasFloSecCHW(redeclare
      package                                                                    Medium =
        MediumCHW) annotation (Placement(transformation(
        extent={{10,12},{-10,-12}},
        rotation=180,
...
equation
  ...
  connect(chillerStage.On, realToBoolean.y) annotation (Line(
      points={{-84,76},{-120,76},{-120,90},{-139,90}},
      color={255,0,255},
      pattern=LinePattern.Dash));
  connect(chillerStage.Status, mulChiSys.Rat) annotation (Line(
      points={{-84,60},{-90,60},{-90,36},{-44,36},{-44,-16.6},{-54.1,-16.6}},
      color={0,0,127},
      pattern=LinePattern.Dash));
  ...
  connect(pumSecCHW.SpeRat, secPumCon.Status) annotation (Line(
      points={{79.7,-78.84},{88,-78.84},{88,-14},{30,-14},{30,44},{58,44}},
      color={0,0,127},
      pattern=LinePattern.Dash));
  ...
  connect(Loa.y, chillerStage.Loa) annotation (Line(
      points={{-153,26},{-124,26},{-124,68},{-84,68}},
      color={0,0,127},
      pattern=LinePattern.Dash));
