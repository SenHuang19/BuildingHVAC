...
model BoilerPlant
  ...
  Subsystems.Pump.PumpSystem pumSecHW(
    ...
    PreCur=pressure,
    dp={dP_nominal*0.25 for i in linspace(
        1,
        m,
        m)}) annotation (Placement(transformation(extent={{22,0},{-12,36}})));
  ...
  IBPSA.Fluid.Sensors.TemperatureTwoPort senTHWBuiLea(
    ...
    m_flow_nominal=sum(mHW_flow_nominal)) annotation (Placement(transformation(
        ...
        rotation=0,
        origin={120,18})));
  replaceable IBPSA.Fluid.Sensors.MassFlowRate senMasFlo(redeclare package Medium =
...
equation
  ...
  connect(pumSecHW.SpeRat, secPumCon.Status) annotation (Line(
      points={{-13.7,29.16},{-20,29.16},{-20,42},{58,42}},
      color={0,0,127},
      pattern=LinePattern.Dash));
  ...
  connect(secPumCon.y, pumSecHW.SpeSig) annotation (Line(
      points={{81,50},{81,50},{122,50},{122,32.4},{23.53,32.4}},
      color={0,0,127},
      pattern=LinePattern.Dash));
