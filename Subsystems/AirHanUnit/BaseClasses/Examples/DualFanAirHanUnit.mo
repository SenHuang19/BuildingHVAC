...
model DualFanAirHanUnit
  ...
  package MediumAir = IBPSA.Media.Air "Medium model";
  package MediumWat = IBPSA.Media.Water "Medium model";
  parameter Modelica.SIunits.MassFlowRate mAirFloRat = 18.3*1.2 "mass flow rate for air";
  parameter Modelica.SIunits.MassFlowRate mWatFloRat = 18.3*1.2*11.1/6.67/4.2 "mass flow rate for water";
  parameter Modelica.SIunits.Pressure PreDroCoiAir = 50 "Pressure drop in the air side";
  parameter Modelica.SIunits.Pressure PreDroMixingBoxAir = 100 "Pressure drop in the air side";
  parameter Modelica.SIunits.Pressure PreDroWat = 79712 "Pressure drop in the water side";
  parameter Real Coi_k(min=0, unit="1") = 1 "Gain of controller";
  parameter Modelica.SIunits.Time Coi_Ti(min=Modelica.Constants.small)=0.5
    "Time constant of Integrator block";


  parameter Modelica.SIunits.MassFlowRate mFreAirFloRat = 18.3*1.2*0.5 "mass flow rate for fresh air";
  ...
  parameter Modelica.SIunits.VolumeFlowRate VolFloCur[:] = {10,15,20,20*2} "Volume flow rate curve";
  parameter Modelica.SIunits.Pressure SupPreCur[:] = {900,700,500,500*0.5} "Pressure curve";
  parameter Modelica.SIunits.Pressure RetPreCur[:] = {200,150,100,50} "Pressure curve";
  ...
  IBPSA.Fluid.Sensors.TemperatureTwoPort senTem(redeclare package Medium = MediumAir, m_flow_nominal=mAirFloRat)
  ...
  IBPSA.Fluid.Sensors.RelativePressure senRelPre(redeclare package Medium = MediumAir)
