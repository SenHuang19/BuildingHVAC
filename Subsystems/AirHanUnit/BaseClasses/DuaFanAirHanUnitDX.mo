...
model DuaFanAirHanUnitDX
  ...
  parameter Modelica.SIunits.MassFlowRate mFreAirFloRat "mass flow rate for fresh air";
  parameter Modelica.SIunits.Temperature TemEcoHig "the highest temeprature when the economizer is on";
  parameter Modelica.SIunits.Temperature TemEcoLow "the lowest temeprature when the economizer is on";
  parameter Real MixingBoxDamMin "the minimum damper postion";
  parameter Real MixingBox_k(min=0, unit="1") = 1 "Gain of controller";
  parameter Modelica.SIunits.Time MixingBox_Ti(min=Modelica.Constants.small)=0.5
    "Time constant of Integrator block";

  parameter Real Fan_k(min=0, unit="1") = 1 "Gain of controller";
  parameter Modelica.SIunits.Time Fan_Ti(min=Modelica.Constants.small)=0.5
    "Time constant of Integrator block";
  ...
  IBPSA.Fluid.Sensors.TemperatureTwoPort senTemDisAir(redeclare package Medium =
  ...
  IBPSA.Utilities.IO.SignalExchange.Read pFanRet
    annotation (Placement(transformation(extent={{-8,-58},{-28,-38}})));
  IBPSA.Utilities.IO.SignalExchange.Read TDis
...
equation
  ...
  connect(dxCoil.TConIn, TOut) annotation (Line(
      ...
      color={0,0,127},
      pattern=LinePattern.Dash));
  connect(supFan.P, pFanSup.u) annotation (Line(
      points={{49,4},{62,4},{62,26},{-32,26},{-32,40},{-22,40}},
      color={0,0,127},
      pattern=LinePattern.Dash));
  connect(retFan.P, pFanRet.u) annotation (Line(
      points={{-31,-74},{-52,-74},{-52,-58},{6,-58},{6,-48},{-6,-48}},
      color={0,0,127},
      pattern=LinePattern.Dash));
  connect(TDis.u, senTemDisAir.T) annotation (Line(
      points={{70,-48},{56,-48},{56,-22},{68,-22},{68,22},{82,22},{82,11}},
      color={0,0,127},
      pattern=LinePattern.Dash));
